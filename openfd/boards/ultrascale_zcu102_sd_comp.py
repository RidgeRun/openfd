...
# ==========================================================================
#
# Copyright (C) 2016, LLC (http://www.ridgerun.com)
#
# Authors: Luis Diego Chaverri Masis <diego.chaverri@ridgerun.com>
#
...
class Ultrascale_Zcu102SdCompInstaller(object):
    """
    ...
    def install_uboot(self, mount_point):
        """
        Copies the u-boot BOOT.bin to the given mount point.
        ...
        cmd = 'sudo cp %s %s/BOOT.bin' % (self._uboot_file, mount_point)
    ...
    def install_kernel(self, mount_point):
        ...
        cmd = 'sudo cp %s %s/Image' % (self._kernel_image, mount_point)
        if self._e.check_call(cmd) != 0:
            raise BoardError('Failed copying %s to %s' %
