...
class Tx1SdCompInstaller(object):
    """
    ...
    def install_kernel(self, mount_point):
        ...
        # Create /boot directory if doesn't exist
        cmd = 'sudo mkdir -p %s/boot/' % (mount_point)
        if self._e.check_call(cmd) != 0:
            raise BoardError('Failed creating directory %s/boot' %
                               (mount_point))
        
        cmd = 'sudo cp %s %s/boot/%s' % (self._kernel_image, mount_point, self.kernel_file_type)
        if self._e.check_call(cmd) != 0:
            raise BoardError('Failed copying %s to %s' %
    ...
    def install_kernel_devicetree(self, mount_point):
        ...
        cmd = 'sudo cp %s %s/boot/' % (self._kernel_devicetree, mount_point)
        if self._e.check_call(cmd) != 0:
            raise BoardError('Failed copying %s to %s' %
